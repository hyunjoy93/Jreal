...
<section class="section">
	...
	<div class="conHead">
		...
		<div class="headline">
			<h2 class="pageTitle">점검통계(업체별)</h2>
		</div>
							...
							<tr>
								...
								<td style="padding-right:4px">
									<select class="select md" id="searchType" >
										<option value="corpKoNm">업체명</option>
								...
								<td>
 									...
 									<p id="dateContainer">	
 										<input id="chkStrtDt" type="text" class="input date" placeholder="시작일자" readonly> 
 										~ 
 										<input id="chkEndDt"  type="text" class="input date" placeholder="종료일자" readonly>
								    </p> 
								</td>
							</tr>
						</tbody>
...
function fnInitGrid(){
	...
	// 그리드 컴럼 정보
	var columnsData = [
			  {header:'업체명', name:'corp_ko_nm', align:'left', width:300}
			, {header:'정기점검', name:'chk_1', align:'center'}
			, {header:'장애처리', name:'chk_2', align:'center'}
			, {header:'긴급점검', name:'chk_3', align:'center'}
// 			, {header:'기타점검', name:'chk_4', align:'center'}
...
function fnList(slcPageNo){
	...
	var params = {
		...
		chkStrtDt  : $("#chkStrtDt").val().replace(/-/g,""),
		chkEndDt   : $("#chkEndDt").val().replace(/-/g,"")
	};
    
	// GRID 리스트 조회
	fnAjax("<c:url value='/api/tot/totChkCorpList'/>", params, function(data) {

		if(successResCode == data.resCode){
			// RESET DATA(그리드)
			gridObj["grid"].resetData(data.TotChkCorpList);
...
fnAjax("<c:url value='/api/tot/totChkCorpList'/>", params, function(data) {
  if (successResCode == data.resCode) {
    var totChkCorpList = data.TotChkCorpList;
    var chartLabel = []; // 라벨 데이터를 담을 배열

    // 라벨 데이터 생성
    for (var idx in data.TotChkCorpList) {
      if (data.TotChkCorpList[idx].corp_ko_nm == "Total") {
        continue;
      }
      corpKoNm = String(data.TotChkCorpList[idx].corp_ko_nm);
      chartLabel.push($.trim(corpKoNm));
    }

    // 정기정검 데이터 생성
    var chk_1 = [];
    for (var idx in data.TotChkCorpList) {
      chk_1.push(data.TotChkCorpList[idx].chk_1);
    }
    // 장애처리 데이터 생성
    var chk_2 = [];
    for (var idx in data.TotChkCorpList) {
      chk_2.push(data.TotChkCorpList[idx].chk_2);
    }
    // 긴급점검 데이터 생성
    var chk_3 = [];
    for (var idx in data.TotChkCorpList) {
      chk_3.push(data.TotChkCorpList[idx].chk_3);
    }
    // 기타정검 데이터 생성
    var chk_4 = [];
    for (var idx in data.TotChkCorpList) {
      chk_4.push(data.TotChkCorpList[idx].chk_4);
    }
    ...
    var myChart = new Chart(ctx, {
		...
		data: {
			...
			datasets: [{
				label: '정기점검',
				data: chk_1,
// 				data: [1,6,3,4,2,2,2,1],
				backgroundColor: 'rgb(0,19,76)',
//            	    backgroundColor: "rgba(240, 159, 64, 0.5)", // 블루톤 색상
				},
				{
				label: '장애처리',
				data: chk_2,
// 				data: [1,2,9,4,2,15,2,1],
